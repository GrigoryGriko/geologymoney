...
<?php
    if ($_SESSION['ADMIN_LOGIN_IN']) {
        ...
        $db->Query("SELECT SUM(`money_payin`) AS `sum_payin` FROM `history_money_payin` WHERE `uid` != 1 AND `bot` = 0"); //
        ...
        echo '<br><br>Пополнено людьми: <b style="color: green; font-size: 24px;">'.$as_money_payin_real['sum_payin'].' руб.</b>(из них на рекламу: '.$as_money_ad['sum_payin'].' руб. - из которых заработано системой: <b style="color: green; font-size: 24px;">'.$total_system_earn.' руб.)</b>|| ';
        echo 'Для Статистики: '.$as_money_payin['sum_payin'].' руб.';
