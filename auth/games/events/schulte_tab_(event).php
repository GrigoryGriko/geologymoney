...
if ( !empty($NumRows) ) {
    ...
    if ($as_games_data['user_create_id'] == $_SESSION['id'] and $as_games_data['ready_play_user_2'] == 0) {
        ...
        if ($_GET['action'] == 'delete') {     //если есть запрос на удаление игры
            ...
            MessageSend('Игра удалена', '/schulte_tab(start)');
        }
        ...
        if ( !empty($NumRows) ) {
            ...
            if ($assoc_u_d['balance_game'] >= $as_games_data['sum_bet']) {
                ...
                MessageSend('Недостаточно средств', '/schulte_tab(start)');    
            }
        }
        else {
            MessageSend('Вас не существует', '/schulte_tab(start)');
        }
        ...
        MessageSend('Игра завершена', '/schulte_tab(start)');
    }
}
else {
    MessageSend('Доступ запрещен', '/schulte_tab(start)');
}
?>
<?php
...
echo '
    <script type="text/javascript">
        ...
        $(document).on("click", ".param_blocks", function() { 
            ...
            $.get("request/schulte_tab_request/schulte_tab_request_GET(data)", function(data) {    //функция получает данные data с файла по директории
                data = $(data);
            ...
            <div class="centerer_e1s3_center_panel">    
                <div class="element_1_stroke_3_box_1_center_panel">
                    ...
                    <div class="box_elements_e1s3b1cp">
                        <a href="/schulte_tab(start)"><div class="back"><img src="auth/games/events/img/schulte_tab/back.png"></div></a>
