...
?>
<?php
top_auth('Заказать выплату', '/auth/pay/style/payout_moneystyle', false, 'payout_money');
?>
...
for ($id3 = 1; $id3 <= $ps_count; $id3++) {
	...
	echo '
				<form id="'.$form_id3.'" class="form_class" style="display: none" name="'.$name_id3.'" method="post" action="/actions/getmoney">
...
echo '
					ВАШИ ПОСЛЕДНИЕ ВЫПЛАТЫ<br>
					ПОСЛЕДНИЕ 100 ВЫПЛАТ.<br>
                    <p style="text-transform: none; font-weight: 400">(Выплаты обрабатываются 5 минут)</p>';
echo '
				</div>
...
$db->Query("SELECT * FROM `history_money_payout` WHERE `uid` = '$_SESSION[id]' ORDER BY `date` DESC");
