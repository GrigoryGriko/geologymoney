        ...
        if ( iconv_strlen($_POST['sum_payout']) < 61 ) {
            if ($_POST['sum_payout'] > 0) {
                ...
                if ( $_POST['sum_payout'] < $assoc_wallet_info['min_sum']) {
                    MessageSend('Сумма выплаты меньше допустимого значения', '/pay/payout_money');
                }
                else if ($_POST['sum_payout'] > $assoc_wallet_info['max_sum']) {
                    MessageSend('Сумма выплаты больше допустимого значения', '/pay/payout_money');
                }
                else {
                    ...
                    if ( !empty($NumRows) ) {
                        ...
                        if ( !empty(NumRows_stats) ) {
                            ...
                            if ($_POST['sum_payout'] <= $assoc_users_data['balance_withdrawal']) {

                                $db->Query("SELECT * FROM `history_money_payout` WHERE `uid` != '$_SESSION[id]' AND `account_wallet` = '$_POST[acount_wallet]'");
                                $numrows = $db->NumRows();
                                if ( empty($numrows) ) {

                                    $db->Query("UPDATE `users_data` SET `balance_withdrawal` = (`balance_withdrawal` - '$_POST[sum_payout]') WHERE `uid` = '$_SESSION[id]'");

                                    $db->Query('INSERT INTO `history_money_payout` VALUES (NULL, "'.$_SESSION['id'].'", "'.$_POST['sum_payout'].'", "'.$name_ps.'", "'.$_POST['acount_wallet'].'", NOW(), 0, 0, "'.$assoc_users_data['balance_buy'].'", "'.$assoc_users_stats['money_payin'].'", "'.$assoc_users_stats['money_payout'].'", "'.$assoc_users_stats['money_earn_refs_total'].'", "'.$assoc_us['date_reg'].'", 0, "'.$_SESSION['email'].'")');

                                    MessageSend('Ваша заявка на выплату успешно создана. Обработка занимает до 5 минут', '/pay/payout_money');

                                }
                                else {
                                    MessageSend('На данный номер кошелька производилась выплата с другого аккаунта. Обратитесь в техподдержку.', '/pay/payout_money');  
                                }     
                            }
                            else {
                                MessageSend('Недостаточно средств для вывода', '/pay/payout_money'); 
                            }
                        }
                        else {
                            MessageSend('Недостаточно средств для вывода', '/pay/payout_money');    
                        ...
                        MessageSend('Вы не авторизованы', '/pay/payout_money');
                    }
                }
            }
            else {
                MessageSend('Введите корректную сумму', '/pay/payout_money');
            }
        }
        else {
            MessageSend('Сумма слишком большая', '/pay/payout_money');
        }
    }
    else {
        MessageSend('Укажите сумму для выплаты', '/pay/payout_money');
    }
}
            ...
            if ($_POST['payment_password']) {
                if (iconv_strlen($_POST['payment_password']) < 61) {
                    if ( $assoc_users['payment_password'] != hash(sha256, $_POST['payment_password']) ) {
                        MessageSend('Платежный пароль введен неверно', '/pay/payout_money');    //exit скрипта
                    }
                }
                else {
                    MessageSend('Платежный пароль слишком длинный', '/pay/payout_money');
                }
            }
            else {
                MessageSend('Введите платежный пароль', '/pay/payout_money');
            }
        }
        ...
        if ($_POST['payin_sys_1']) {    //payeer
            if ($_POST['acount_wallet']) {
                ...
                if ( $account_wallet_valid == true) {
                                        ...
                                        if ( !empty(NumRows_stats) ) {
                                            ...
                                            if ($_POST['sum_payout'] <= $assoc_users_data['balance_withdrawal']) {
                                                ...
                                                if ( empty($numrows) ) {
                                                    ...
                                                    $db->Query('INSERT INTO `history_money_payout` VALUES (NULL, "'.$_SESSION['id'].'", "'.$_POST['sum_payout'].'", "'.$name_ps.'", "'.$_POST['acount_wallet'].'", NOW(), 0, 0, "'.$assoc_users_data['balance_buy'].'", "'.$assoc_users_stats['money_payin'].'", "'.$assoc_users_stats['money_payout'].'", "'.$assoc_users_stats['money_earn_refs_total'].'", "'.$assoc_users_data['date_reg'].'", 0, "'.$_SESSION['email'].'")');

                                                    MessageSend('Ваша заявка на выплату успешно создана', '/pay/payout_money');
                    ...
                    MessageSend('Номер кошелька не соответсвует формату', '/pay/payout_money');
                }
            }
            else {
                MessageSend('Укажите номер кошелька для выплаты', '/pay/payout_money');
            }
        }
/*add payin_sys 2*/
            ...
            MessageSend('Данная платежная система недоступна', '/pay/payout_money');
        }   

    }
    else {
        MessageSend('Для выплаты подтвердите свой e-mail в&nbsp<a href="/setting_account">настройках аккаунта</a>', '/pay/payout_money');
    }
}
else {
    MessageSend('Вы не авторизованы', '/pay/payout_money');
