...
if ($_POST['mineralscollect_f'] and $_POST['confirm'] == 1) {
	...
	if ( !empty($row_users_amount_mine) ) {
		while ( $assoc_u_a_m = $db->FetchAssoc() ) {	//запись массива из конкретного запроса
			if ( (time() - strtotime($assoc_u_a_m['date_collection']) ) >= 600) {		//3600 временно из-за непонятной проблемы со временем
				...
				if ( isset($ident_name_mineral) ) {
					//PAGE_leadrace_payin_refresh();	//обновлять минералы за лидерство
		...
		switch($indicator) {
			case 1:
				message('Вы успешно собрали минералы');
				break;
			case 0:
				message('Собирать минералы можно 1 раз в 10 минут', false, false, 'info');
				break;
			default:
	...
	if ( !empty($row_users_amount_mine) ) {
		...
		if ($assoc_u_a_m['level'] < 7) {
			$assoc_u_a_m_level_up = $assoc_u_a_m['level'] + 1;	//уровень рудника на 1 выше от текущего

			$db->Query("SELECT * FROM `mine_in_shop` WHERE `category` = $assoc_u_a_m[category] AND `level` = ".$assoc_u_a_m_level_up."");		//данные рудника следующего уровня
			...
			if ( !empty($row_mine_in_shop) ) {
				...
				if ( !empty($row_users_data) ) {
					...
					if ($assoc_users_data['balance_buy'] >= $assoc_mine_in_shop['price']) {		//дополнить эту часть кода
						...
						message('Вы успешно подняли уровень рудника до '.$assoc_u_a_m_level_up.'-го');
					}
					else {
			...
			message('Уровень вашего рудника максимальный', false, false, 'info');	
		}
	}
	else {
		message('Fatal error', false, false, 'error');
